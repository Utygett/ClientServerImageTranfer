...
bool TCPTransferData::send(const QByteArray& data, const QString& imageType)
{
    ...
    char pImageType[IMAGE_TYPE_SIZE];
    pImageType[0] = '\0';
    ...
    strncpy(pImageType,imageType.toStdString().c_str(),IMAGE_TYPE_SIZE);
    m_socket.write(reinterpret_cast<char *>(&dataSize), 4);
    m_socket.write(pImageType, IMAGE_TYPE_SIZE);
    m_socket.write(reinterpret_cast<const char *>(data.data()), dataSize);
    strncpy_s(pImageType, IMAGE_TYPE_SIZE,imageType.toStdString().c_str(), IMAGE_TYPE_SIZE);
